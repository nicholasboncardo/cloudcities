<script>
	...
	onMount(async () => {
		...
		let data = await fetch("https://cloudcities.studiotomassaraceno.org/wp-json/wp/v2/media?categories=48")
	...
	<main>
			...
			{/if}
			{#if !drawApp}
				<Overlay openComponent={component} image={chosenImage} on:goToAbout={goToAbout} on:closeLargeImage={closeIndividualPost} on:clickDraw={openDrawing}/>
			{/if}
